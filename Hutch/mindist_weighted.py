# Program: mindist_weighted.py
#
# Purpose: For each file of aligned participant sequences, calculates a
#          consensus sequence for each subject, measures the pairwise weighted
#          Hamming distances between each sequence and the consensus sequence,
#          and selects the sequence with the shortest distance between them.
#
...
def main(json_fp):
    ...
    JSON file parameters:
        participant_file_dir - The directory containing the participant
                               sequences.
        refseq - The reference sequence identifier, if the reference sequence
                 is included in the participant sequences.
        matrix_file - The file containing the weighted Hamming distances.
    ...
    \b
    Execution: python mindist_weighted.py <JSON file>
    ...
    out_stat_fp.write("seq_id,weighted_hamming_distance\n")

    # Read the matrix file into a dictionary.
    with open (json_dict["matrix_file"], "r") as matrix_fp:
        matrix_df = pd.read_csv(matrix_fp)
        matrix_df = matrix_df.set_index("aa1")
    ...
    for seq_file in seq_file_list:
        ...
        # Cycle through the sequences in the file and get the one closes to the
        # consensus sequence.
        min_score = 0
        min_seq_dict = {}
        for seq_id in seq_dict:
            ...
            for nt in range(len(seq_dict[seq_id])):
                n_seq = seq_dict[seq_id][nt]
                n_cons = consensus_seq[nt]
                distance += float(matrix_df.at[n_seq, n_cons])
